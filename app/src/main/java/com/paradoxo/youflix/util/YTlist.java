...
public class YTlist {
    ...
    public static void main(String[] args) {
        try {
            YTlist.listaVideos(new PaginaVideo(), false);
        } catch (Exception e) {
            e.printStackTrace();
        }
    }
    ...
    private static Video infoVideo(String idVideo) {
        ...
        try {
            ...
            if (conteudoVideo != null) { // Se existir resultados:
                ...
                String temp;
                try {
                    temp = ((Thumbnail) busca.getItems().get(0).getSnippet().getThumbnails().get("maxres")).getUrl(); // Só pra testar um possivel erro casso não exitsa uma thumb desse tamanho
                    ...
                    try {
                        temp = ((Thumbnail) busca.getItems().get(0).getSnippet().getThumbnails().get("standard")).getUrl(); // Só pra testar um possivel erro casso não exitsa uma thumb desse tamanho
